...
class MothBrainSigmoid(nengo.Network):
    def __init__(self, weight_noise=0.01, sigmoid_noise=0.01, inhib=1):
        ...
        self.W_in_GIIC = noisify_weights(np.array([[1, 0],
                                              [0, 1]]), noise=weight_noise) # input onto contralateral GII-C
        ...
        self.W_GIIC_motor = noisify_weights(np.array([[-inhib, 0],
                                                 [0, -inhib]]), noise=weight_noise) # GII-C onto ipsilateral motor
        ...
        self.out_GIIC = np.array([0.3,0.3]) # GII-C cell: bg freq not given (but low), max freq ~40 Hz
    ...
    def update(self, t, x):
        ...
        # Calculate GII-C activity
        self.out_GIIC = noisy_sigmoid(np.dot(self.W_in_GIIC,v[0])+self.out_GIIC,0.5,0.7,noise=self.noise)
        #GIIA = np.vstack((GIIC,out_GIIC))
        
        # Calculate motor activity
        input_2_motor = np.dot(self.W_FF_motor,self.FF_mem)+np.dot(self.W_GIIC_motor,self.out_GIIC)
